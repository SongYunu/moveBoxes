"""Resumable state ACT training. No simulator or renderer occupies training VRAM."""
import json
import math
import os
import random
...
def validate(model, data, cfg, device):
    ...
    for _ in range(cfg['validation_batches']):
        obs, actions, mask = data.batch(cfg['batch_size'], generator, device)
        pred, kl = model(obs)  # Same zero-latent prior as deployment, not posterior loss.
        loss, _ = action_loss(pred, actions, mask, kl, cfg['kl_weight'])
...
def train(job):
    ...
    trajectories = load_trajectories(job['data'], job.get('num_demos'))
    ...
    train_ids, val_ids = split_trajectories(trajectories, cfg['seed'])
    train_data = WindowData(trajectories, train_ids, arch['history'], arch['chunk_size'])
    val_data = WindowData(trajectories, val_ids, arch['history'], arch['chunk_size'])
    ...
    print(f"ACT ver2: {params/1e6:.2f}M parameters / {device} / AMP={amp} / resume {start}/{cfg['total_iters']}", flush=True)
    ...
    for step in range(start, cfg['total_iters']):
        ...
        for group in optimizer.param_groups:
            group['lr'] = lr
        obs, actions, mask = train_data.batch(cfg['batch_size'], generator, device, cfg['position_noise'])
        optimizer.zero_grad(set_to_none=True)
        with torch.amp.autocast(device_type=device.type, enabled=amp):
            prediction, kl = model(obs, actions, mask)
        ...
        if done % cfg['save_freq'] == 0 or done == cfg['total_iters']:
            ...
            weights = dict(format='moveboxes-act-ver2', model_config=arch, model=model.state_dict(), step=done)
            if val_loss < best:
                best = val_loss
