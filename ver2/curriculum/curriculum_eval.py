"""Unchanged official score plus measured action counts, with efficiency reported only for full successes."""
import json
import random
import sys
import time
...
METRICS = ('sort_accuracy','mean_sorted','all_placed_rate','mean_steps','mis_sort_rate','mean_completion_actions')
...
class Observer(NextPickObserver):
    ...
    def capture_progress(self, action_count):
        if self.env is None:return  # Video recording owns a separate environment; no score is collected there.
        count=int(self.env.unwrapped.evaluate()['success_count'][0].item())
        for k in range(1,count+1):self.completion_steps.setdefault(str(k),action_count)

    def act(self, obs, deterministic=True):
        self.capture_progress(len(self.decisions))
    ...
    def report(self):
        result = super().report()
        result['sort_completion_steps'] = dict(self.completion_steps)
        result['stages'] = dict(
    ...
    try:
        ...
        if not job.get('video_only'):
            for seed in job['seeds'][len(rows):]:
                ...
                agent.capture_progress(job['max_steps']-1)
                correct=int(metrics['mean_sorted'])
                metrics['mean_completion_actions']=agent.completion_steps.get(str(correct),job['max_steps']-1)
                row = dict(seed=seed, **metrics, next_pick=agent.report())
                row['successful_actions']=agent.completion_steps.get('4') if correct==4 else None
                if agent.trace:
                    ...
                    successful=[r['successful_actions'] for r in rows if r['successful_actions'] is not None]
                    save_json(job['output'], dict(average, mean_successful_actions=sum(successful)/len(successful) if successful else None,
