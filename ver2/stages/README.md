...
새 기본 `run_name`은 `moveboxes_stage_act_v1`입니다. 기존 ACT/DP 체크포인트는 구조가 달라 사용할 수 없습니다.
...
로컬 CPU에서 신규 8개 + 기존 32개 테스트, 세 난이도 실제 데이터 각 4개 시연의 전체 크기 모델 forward/backward를 확인했습니다.
**Colab T4의 실제 물리 시뮬레이션, 수집 성공률, 점수 향상은 아직 검증하지 않았습니다.** 06/11/16의 짧은 실행부터 확인하세요.
