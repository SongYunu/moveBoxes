"""Partial-success curriculum trainer; original v2 training path remains unchanged."""
import json
import math
import os
import random
...
from curriculum_data import load_data, split_data, normalization, StageWindows
from marso_experiment import save_json, digest, read_json
...
def train(job):
    ...
    train_data = StageWindows(trajectories, train_ids, arch['history'], arch['chunk_size'],
                              first_pick_fraction=cfg.get('first_pick_fraction',0.),efficiency_bonus=cfg.get('efficiency_bonus',.5))
    ...
    save_json(folder/'data_audit.json', dict(
        ...
        labeling='deadline-verified collection only: observed state + exact grasp flag',
        ...
        first_pick_fraction=0., contact_sampling_fraction=.35, evaluation_actions=199,
        curriculum_minimum_correct=read_json(job['data'])['minimum_correct'],
        efficiency_bonus=cfg.get('efficiency_bonus',.5), efficiency_weights=train_data.efficiency,
        efficiency_rule='only fully successful demonstrations; time scaling uses training split only',
        four_box_demonstrations=sum(e['fully_successful'] for e in read_json(job['data'])['episodes']),
    ...
    for step in range(start, stop):
        ...
        with torch.amp.autocast(device_type=device.type, enabled=amp):
            ...
            loss, parts = stage_loss(outputs, batch['actions'], batch['mask']*batch['efficiency_weight'][:,None], batch['stage'], batch['gate'], cfg)
