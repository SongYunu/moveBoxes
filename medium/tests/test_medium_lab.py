...
from build_medium_notebook import CONFIG,make_notebook
from stage_train import train
from stage_data import load_data,split_data,StageWindows
def state():
    obs=np.zeros(72,dtype=np.float32)
    obs[18:21]=[.2,0,.22]
    obs[26:54]=np.asarray([[.2,i*.08,.03,1,0,0,0] for i in range(4)]).flatten()
    obs[54:62]=[1,0,0,1,1,0,0,1]
    obs[62:68]=[0,-.36,0,0,.36,0]
    return obs


class MediumLabTests(unittest.TestCase):
    def test_success_requires_all_four_boxes(self):
        result=quality(dict(sort_accuracy=.875,episodes=[
            dict(mean_sorted=3,next_pick=dict(stable_grasp_cycles=4)),
            dict(mean_sorted=4,next_pick=dict(stable_grasp_cycles=4))]))
        self.assertEqual(result['all_sorted_rate'],.5)
        self.assertEqual(result['score'],.875)

    def test_notebook_runs_medium_only_and_keeps_stages_notebook(self):
        nb=make_notebook()
    ...
    def test_training_and_eval_resume_at_block_boundary_and_preserve_best(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            exp=MediumLab(cfg,source_bundle());exp.connect();exp.repo.mkdir()
            path=exp.data/'medium/trajectory.state.pd_ee_delta_pos.physx_cuda.h5';path.parent.mkdir(parents=True)
            ...
            folder=exp.run_dir/'medium';folder.mkdir()
            ...
            def run(command,cwd=None,log=None):
                ...
                if command[1]=='stage_train.py':
                    with patch('stage_train.sync_from_env'):
                        train(job)
                    return
                self.assertEqual(job['level'],'medium')
                ...
                row=dict(sort_accuracy=score,mean_sorted=4*score,all_placed_rate=float(score==1),mean_steps=120,
                    mis_sort_rate=0,next_pick=dict(stable_grasp_cycles=int(4*score),gripper_sign_reversals=2))
            ...
            with patch.object(exp,'training_job',side_effect=job_without_collection),patch.object(exp,'run',side_effect=run):
                ...
                self.assertEqual(exp._test_candidate('medium')[0].name,'block_02.pt')
            ...
            with self.assertRaisesRegex(ValueError,'Medium only'):
