"""Easy only: repeated short training and actual simulator feedback."""
import json
import sys
...
CONFIG = dict(STAGE_CONFIG,run_name='moveboxes_easy_lab_v1',source_run_name='moveboxes_stage_act_v1',
...
def make_notebook(config=None):
    ...
    lines = ['# 01 · EASY 집중 실험 CONFIG','CFG = {']
    ...
    nb['cells']=nb['cells'][:4]
    tasks = [
        ('05 · Easy GPU 확인 / 기존 성공 시연과 모델 가져오기', 'experiment.check_runtime()\nexperiment.prepare()'),
        ('06 · EASY 반복 학습 · 중단 후 같은 셀을 다시 실행하면 이어서 진행', 'history = experiment.run_blocks()'),
        ('07 · 최고 모델 8회 별도 테스트 + 영상 + 2회 행동 기록', 'experiment.test("easy")\nexperiment.diagnose("easy")'),
        ('08 · 학습 곡선과 최고 결과', 'experiment.report()'),
        ('09 · 목표 달성 모델의 별도 시드 최종 평가', 'experiment.final_evaluation()'),
        ('10 · 평가 완료한 Easy 모델만 패키징', 'experiment.package()')]
    for i,(title,body) in enumerate(tasks,5):
        nb['cells'].append(dict(cell_type='code',execution_count=None,metadata={'id':f'easy-{i}'},outputs=[],
            source=(f'# {title}\n{body}\n').splitlines(keepends=True)))
    nb['metadata']['colab']['name']='moveboxes_easy_colab.ipynb'
    return nb
...
if __name__ == '__main__':
    ...
    path=Path(__file__).parents[1]/'notebooks/moveboxes_easy_colab.ipynb'
