"""Easy-only bounded training blocks, simulator selection and resumable results."""
import hashlib
import json
import os
import shutil
...
def quality(metrics):
    ...
    both = sum(r['mean_sorted'] >= 2 for r in rows)/len(rows)
    return dict(score=metrics['sort_accuracy'], first_grasp_rate=first, both_sorted_rate=both)


def rank(result):
    return result['score'],result['both_sorted_rate'],result['first_grasp_rate']


class EasyLab(StageExperiment):
    def __init__(self, cfg, sources):
        super().__init__(cfg,sources)
        c = self.cfg
        c['action_training_mode'] = 'prior'
        c['total_iters'] = dict(easy=c['block_iters']*c['max_blocks'],medium=1,hard=1)
        ...
        if min(c['block_iters'],c['max_blocks'],c['development_episodes'],c['final_episodes'],
            ...
            raise ValueError('Easy experiment budgets must be positive')
        ...
        if c['max_episode_steps']['easy'] != 200:
            raise ValueError('Easy lab keeps the 200-step evaluation protocol')

    def notebook_snapshot(self):
        from build_easy_notebook import make_notebook
...
try:
    ...
    env.close()
"""
        self.run([sys.executable,'-c',code],cwd=self.repo,log=self.session/'easy_gpu_check.log')

    def prepare(self):
        self._ready()
        self.prepare_data()
        folder = self.run_dir/'easy'
        with self.persist_operation('easy'):
            ...
            if not ready and self.cfg.get('source_run_name'):
                ...
                snapshots = sorted(n for n in store.assets if n.startswith('snapshot-easy-'))
                if not snapshots:
                    raise FileNotFoundError('기존 실행에 Easy snapshot이 없습니다.')
                ...
                for entry in snapshot['entries']:
                    ...
                    wanted = len(parts)==3 and parts[:2]==('easy','collection') and Path(parts[-1]).suffix in ('.json','.npz')
                    model = entry['path']=='easy/checkpoints/latest.pt' and self.cfg['warm_start']
                    ...
                    if destination.exists():
                        if digest(destination) != entry['sha256']:
                            raise ValueError('Source snapshot changed. Use a new Easy run_name.')
                    else:
            ...
            if not manifest.get('complete'):
                raise RuntimeError('Easy 성공 시연 수집이 미완료입니다.')
            for entry in manifest['episodes']:
                path = safe_target(folder/'collection',entry['file'])
                if digest(path) != entry['sha256']:
                    raise ValueError('Easy collection checksum mismatch')
            if self.cfg['warm_start'] and not (folder/'initial_model.pt').exists():
                raise RuntimeError('기존 Easy 모델이 없습니다. 새로 학습하려면 warm_start=False로 새 실험을 시작하세요.')
        ...
        print(f'Easy 성공 시연 {len(manifest["episodes"])}개 준비 완료. 반복 학습 셀을 실행하세요.')

    def training_job(self):
        folder = self.run_dir/'easy'
        if not (folder/'inputs_ready.json').exists():
            raise RuntimeError('먼저 Easy 시연/모델 준비 셀을 실행하세요.')
        ...
        cfg.update(total_iters=self.cfg['total_iters']['easy'],save_freq=self.cfg['block_iters'],action_training_mode='prior')
        names = ('act_v2_model.py','act_v2_data.py','stage_schema.py','stage_labels.py','stage_model.py','stage_data.py','stage_train.py')
        job = dict(folder=str(folder),data=str(self.data/'easy/trajectory.state.pd_ee_delta_pos.physx_cuda.h5'),
            model_config=self.model_config('easy'),train_config=cfg,policy_config=self.policy_config('easy'),
    ...
    def run_blocks(self):
        ...
        if old and old != identity:
            raise ValueError('Saved Easy protocol differs. Use a new run_name.')
        ...
        if history['status'] in ('target_reached','plateau','budget_reached'):
            print('저장된 Easy 반복 종료 상태:',history['status'])
            return self.report()
        with self.persist_operation('easy'):
            for block in range(len(history['blocks'])+1,self.cfg['max_blocks']+1):
                ...
                for window in self.cfg['ensemble_candidates']:
                    ...
                    policy = dict(self.policy_config('easy'),ensemble_window=window)
                    trial = self._trial('easy',checkpoint,policy,self.seeds(True),label)
                ...
                target = len(recent)==self.cfg['success_streak'] and all(r['best']['both_sorted_rate']>=self.cfg['target_accuracy'] for r in recent)
                ...
                self.sync_level('easy')
                print(f"Easy {stop}회: 첫 집기 {best['first_grasp_rate']:.0%} / 두 상자 정답 {best['both_sorted_rate']:.0%} / 분류 {best['score']:.1%}",flush=True)
    ...
    def _test_candidate(self,level):
        if level != 'easy':
            raise ValueError('This notebook runs Easy only')
        best = read_json(self.run_dir/'easy/lab_best.json')
        if not best:
            return None
        checkpoint = self.run_dir/'easy/checkpoints'/Path(best['checkpoint']).name
    ...
    def report(self):
        self._ready()
        history = read_json(self.run_dir/'easy/lab_history.json',dict(blocks=[],status='not_started'))
        for row in history['blocks']:
            result = row['best']
            print(f"{row['iteration']:5d}회 | 집기 {result['first_grasp_rate']:.0%} | 두 상자 정답 {result['both_sorted_rate']:.0%} | 분류 {result['score']:.1%}")
        print('상태:',history['status'])
        return history

    def final_evaluation(self):
        self._ready()
        selected = self._test_candidate('easy')
        if not selected:
            raise RuntimeError('먼저 Easy 반복 학습을 실행하세요.')
        checkpoint,policy = selected
        best = read_json(self.run_dir/'easy/lab_best.json')
        if best['both_sorted_rate'] < self.cfg['target_accuracy']:
            print('두 상자 성공률이 목표에 못 미쳐 최종 평가는 생략합니다. 진단 셀을 확인하세요.')
            return None
        with self.persist_operation('easy'):
            save_json(self.run_dir/'easy/selection.json',dict(selected=best,tuning_seeds=self.seeds(True)))
            self._trial('easy',checkpoint,policy,self.seeds(False),'metrics')
            return read_json(self.run_dir/'easy/metrics.json')


def source_bundle():
    bundle = stage_sources()
    for name in ('easy_lab.py','build_easy_notebook.py'):
